...
public class DaoBook implements IDaoBook {
    ...
    public List<ModelBook> getSQLSelectAll() {
        return session.selectList("mapper.oracle.mapperBook.getSQLSelectAll");
    }

    @Override
    public List<ModelBook> getSQLSelectLike(String bookname) {
        bookname = StringUtils.isEmpty(bookname) ? null : "%" + bookname + "%" ;
        return session.selectList("mapper.oracle.mapperBook.getSQLSelectLike", bookname );
    }

    @Override
    public List<ModelBook> getSQLSelectEqual(String bookname) {
        return session.selectList("mapper.oracle.mapperBook.getSQLSelectEqual", bookname );
    }

    @Override
    public int setSQLInsert(ModelBook book) {
        return session.insert("mapper.oracle.mapperBook.setSQLInsert", book);        
    }

    @Override
    public int setSQLInsertMulti(List<ModelBook> books) {
        return session.insert("mapper.oracle.mapperBook.setSQLInsertMulti", books);        
    }
    ...
    public int setSQLUpdate(ModelBook updateValue, ModelBook searchValue) {
        ...
        return session.update("mapper.oracle.mapperBook.setSQLUpdate", parameter);
    }
